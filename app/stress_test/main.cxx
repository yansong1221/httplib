...
static std::string format_req_sec(double rps)
{
    if (rps >= 1000)
...
static void parse_url(const std::string& url, std::string& host, uint16_t& port, std::string& path)
{
...
static void print_report(const stress_config& cfg,
                         const std::vector<conn_stats>& conns,
                         std::chrono::nanoseconds total_elapsed)
{
    ...
    auto stdev_of = [](const std::vector<double>& v, double m) {
        if (v.size() < 2)
            return 0.0;
        double sq = 0;
        for (auto x : v)
    ...
    auto pct_near = [](const std::vector<double>& v, double m, double s) {
        if (v.size() < 2 || s <= 0)
            return 100.0;
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()                                                                      //
        ...
        ("url", po::value<std::string>(&cfg.url)->default_value("http://127.0.0.1:8080/"),
         "Target URL (e.g. http://host:port/path)")
        ("body", po::value<std::string>(&cfg.body)->default_value(""), "Request body")      //
        ("method,X",
         po::value<std::string>(&cfg.method)->default_value("GET"),
         "HTTP method")                                                                     //
        ("threads,t", po::value<int>(&cfg.threads)->default_value(2), "Number of threads")  //
        ("connections,c", po::value<int>(&cfg.connections)->default_value(10),
         "Total TCP connections")                                                           //
        ("duration,d", po::value<int>(&cfg.duration_sec)->default_value(10),
         "Duration in seconds")                                                             //
                ...
                while (true) {
                    ...
                    if (result.has_value()) {
                        ...
                        if (ec == boost::system::errc::timed_out
                            || ec == boost::system::errc::operation_would_block)
                            cs.err_timeout++;
