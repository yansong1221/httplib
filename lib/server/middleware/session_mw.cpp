...
std::optional<std::string> session::get(std::string_view key) const
{
...
bool session::has(std::string_view key) const
{
...
const std::unordered_map<std::string, std::string>& session::data() const
{
    return data_;
}
...
public:
    ...
    bool is_expired(const session& s) const { return (clock::now() - s.last_access()) > ttl_; }
};
...
std::shared_ptr<session> memory_session_store::load(std::string_view id)
{
...
void memory_session_store::save(const session& s)
{
    std::lock_guard lock(impl_->mutex_);
    impl_->sessions_[s.id()] = std::make_shared<session>(s);
}

void memory_session_store::destroy(std::string_view id)
{
    std::lock_guard lock(impl_->mutex_);
    impl_->sessions_.erase(std::string(id));
}

void memory_session_store::cleanup()
{
    std::lock_guard lock(impl_->mutex_);
...
public:
    ...
    static void set_cookie(const session_config& cfg, const session& sess, response& resp)
    {
        html::cookie ck;
...
session_middleware::session_middleware(std::shared_ptr<session_store> store)
    : impl_(std::make_unique<impl>())
{
...
session_middleware& session_middleware::cookie_name(std::string name)
{
    impl_->config_.cookie_name = std::move(name);
    return *this;
}

session_middleware& session_middleware::cookie_path(std::string path)
{
    impl_->config_.cookie_path = std::move(path);
    return *this;
}

session_middleware& session_middleware::max_age(std::chrono::seconds age)
{
    impl_->config_.max_age = age;
    return *this;
}

session_middleware& session_middleware::http_only(bool v)
{
    impl_->config_.http_only = v;
    return *this;
}

session_middleware& session_middleware::secure(bool v)
{
    impl_->config_.secure = v;
    return *this;
}

session_middleware& session_middleware::same_site_lax()
{
    impl_->config_.same_site = httplib::html::cookie::same_site_t::lax;
    return *this;
}

session_middleware& session_middleware::same_site_strict()
{
    impl_->config_.same_site = httplib::html::cookie::same_site_t::strict;
    return *this;
}

session_middleware& session_middleware::same_site_none()
{
    impl_->config_.same_site = httplib::html::cookie::same_site_t::none;
    return *this;
}

session_middleware& session_middleware::store_ttl(std::chrono::seconds ttl)
{
    impl_->config_.store_ttl = ttl;
    return *this;
}

bool session_middleware::before(request& req, response&)
{
...
bool session_middleware::after(request& req, response& resp)
{
...
std::shared_ptr<session_store> session_middleware::store()
{
    return impl_->store_;
}
