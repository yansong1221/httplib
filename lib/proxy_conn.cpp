...
proxy_conn::proxy_conn(http_variant_stream_type&& stream,
                       net::ip::tcp::socket&& proxy_socket)
    : stream_(std::move(stream)), proxy_socket_(std::move(proxy_socket))
{
}
void
proxy_conn::read_limit(std::size_t bytes_per_second)
{
    stream_.rate_policy().read_limit(bytes_per_second);
}

void
proxy_conn::write_limit(std::size_t bytes_per_second)
{
