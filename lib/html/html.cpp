...
namespace httplib::html
{
    ...
    namespace detail
    {
        ...
        static void
        _localtime(struct tm* _Tm, time_t const* _Time)
        {
#ifdef _WIN32
        ...
        static void
        _gmtime(struct tm* _Tm, time_t const* _Time)
        {
#ifdef _WIN32
            gmtime_s(_Tm, _Time);
#else
        ...
        static std::string
        to_string(float v, int width, int precision = 3)
        {
            char buf[20] = { 0 };
        ...
        static std::string
        add_suffix(float val, char const* suffix = nullptr)
        {
            std::string ret;

            char const* prefix[] = { "kB", "MB", "GB", "TB" };
            for (auto& i : prefix)
            {
                val /= 1024.f;
        ...
        cast(fs::file_time_type const& endpoint)
        {
            return std::chrono::time_point_cast<std::chrono::system_clock::duration>(
                endpoint - std::filesystem::file_time_type::clock::now() + std::chrono::system_clock::now());
        }

        inline static long
        get_gmt_timezone_offset()
        {
            static long offset = []()
            {
        ...
        make_unc_path(fs::path const& path)
        {
        ...
        inline static std::tuple<std::string, fs::path>
        file_last_wirte_time(fs::path const& file)
        {
        ...
        format_path_list(fs::path const& path, boost::system::error_code& ec)
        {
            std::vector<std::string> path_list;
            std::vector<std::string> file_list;

            for (auto const& file_entry : fs::directory_iterator(path, ec))
            {
                auto const& item = file_entry.path();
    ...
    std::string
    format_dir_to_html(std::string_view target, fs::path const& path, boost::system::error_code& ec)
    {
        ...
        body = head + body + (char const*)detail::tail_fmt;

        return body;
    }
    boost::json::value
    format_dir_to_json(fs::path const& path, boost::system::error_code& ec)
    {
        ...
        for (auto const& file_entry : fs::directory_iterator(path, ec))
        {
            ...
            obj["filename"] = std::string((char const*)file_entry.path().filename().u8string().c_str());
    ...
    std::time_t
    file_last_write_time(fs::path const& path, std::error_code& ec)
    {
    ...
    std::string
    format_http_gmt_date(std::time_t const& time)
    {
        ...
        static std::array<char const*, 7> const WEEKDAY = { "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" };
        static std::array<char const*, 12> const MONTH
    ...
    std::time_t
    parse_http_gmt_date(std::string const& http_date)
    {
        std::tm tm = {};
