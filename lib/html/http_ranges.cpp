...
{
    bool
    http_ranges::parse(std::string_view range_str, size_t file_size)
    {
        ranges_.clear();
        ...
        for (auto const& range : ranges)
        {
            ...
            else
            {
                ...
                {
                    auto [ptr, ec]
                        = std::from_chars(second_range.data(), second_range.data() + second_range.size(), end);
                    if (ec != std::errc {})
                    {
                        ranges_.clear();
                        return false;
    ...
    http_ranges::ranges_type const&
    http_ranges::ranges() const
    {
        return ranges_;
    }

    std::size_t
    http_ranges::size() const
    {
        return ranges_.size();
    }
    ...
    http_ranges::at(std::size_t index) const
    {
        return ranges_.at(index);
    }

    http_ranges::range_type const&
    http_ranges::front() const
    {
        return ranges_.front();
    }

    void
    http_ranges::add(range_type const& range)
    {
