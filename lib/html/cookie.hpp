...
namespace httplib::html
{
    ...
    class cookie_jar
    {
      public:
        ...
        cookie_jar(cookie_jar const& other) : impl_(std::make_unique<impl>(*other.impl_)) {}

        cookie_jar&
        operator=(cookie_jar const& other)
        {
            if (this != &other)
            {
        ...
        parse(std::string_view header)
        {
            ...
            while (!header.empty())
            {
                ...
                if (eq != std::string_view::npos)
                {
                    ...
                    jar.impl_->cookies_.push_back(cookie { .name = std::string(name), .value = std::string(value) });
                }
