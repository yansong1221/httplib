...
typedef std::tuple<bool, coro_http_handler_type, std::unordered_map<std::string, std::string>> coro_result;
...
struct radix_tree_node
{
    ...
    std::shared_ptr<radix_tree_node> insert_child(char index, std::shared_ptr<radix_tree_node> child)
    {
...
public:
    ...
    int coro_insert(const std::string& path, coro_http_handler_type coro_handler, const http::verb& method)
    {
        auto root = this->root;
        ...
        while (i < n)
        {
            if (!root->indices.empty() &&
                (root->indices[0] == type_asterisk || path[i] == type_asterisk ||
                 ...
                 (path[i] == type_colon && root->indices[0] != type_colon) ||
                 (path[i] == type_colon && root->indices[0] == type_colon &&
                  path.substr(i + 1, find_pos(path, type_slash, i) - i - 1) != root->children[0]->path)))
            {
                code = -1;
                break;
            }
            ...
            if (!child)
            {
                ...
                if (p == n)
                {
                    ...
                    root = root->insert_child(path[i], std::make_shared<radix_tree_node>(path.substr(i, p - i)));

                    if (p < n)
                    {
                        root = root->insert_child(type_asterisk, std::make_shared<radix_tree_node>(path.substr(p + 1)));
                        ++param_count;
                    }
                ...
                root = root->insert_child(type_colon, std::make_shared<radix_tree_node>(path.substr(p + 1, i - p - 1)));
                ++param_count;
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (j < m)
                    {
                        std::shared_ptr<radix_tree_node> child(std::make_shared<radix_tree_node>(root->path.substr(j)));
        ...
        if (param_count > this->root->max_params) this->root->max_params = param_count;
    ...
    coro_result get_coro(const std::string& path, const http::verb& method)
    {
        ...
        while (i < n)
        {
            if (root->indices.empty()) return coro_result();
            ...
            else
            {
                ...
                if (!root || path.substr(i, root->path.size()) != root->path) return coro_result();
                i += root->path.size();
            }
        }
