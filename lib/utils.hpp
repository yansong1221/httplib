...
[[nodiscard]] static inline std::string_view mime_type(const std::filesystem::path &path) {
...
static std::string_view buffer_to_string_view(const net::const_buffer &buffer) {
    return std::string_view(net::buffer_cast<const char *>(buffer), net::buffer_size(buffer));
}
