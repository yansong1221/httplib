...
template<typename T>
constexpr inline bool is_basic_stream_v = util::is_specialization_v<std::remove_cvref_t<T>, beast::basic_stream>;
...
public:
    ...
    {
        return std::visit([&, handler = std::move(handler)](auto& t) mutable
    ...
    tcp::endpoint remote_endpoint(boost::system::error_code& ec) { return lowest_layer().remote_endpoint(ec); }
