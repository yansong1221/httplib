...
namespace httplib
{
    ...
    class websocket_stream
    {
      ...
      public:
        ...
        {
            return std::visit([&](auto& t) -> auto const& { return beast::get_lowest_layer(t).socket(); }, stream_);
        }
        ...
        {
            return std::visit([&, handler = std::forward<AcceptHandler>(handler)](auto& t) mutable
                              { return t.async_accept(req, std::forward<AcceptHandler>(handler)); },
                              stream_);
        }
        template <class HandshakeHandler>
        auto
        async_handshake(std::string_view host, std::string_view target, HandshakeHandler&& handler)
        {
            return std::visit([&, handler = std::forward<HandshakeHandler>(handler)](auto& t) mutable
        ...
        set_option(auto&& opt)
        {
            std::visit([opt = std::forward<decltype(opt)>(opt)](auto& s) mutable
        ...
        {
            return std::visit([&, handler = std::forward<CloseHandler>(handler)](auto& t) mutable
                              { return t.async_close(cr, std::forward<CloseHandler>(handler)); },
                              stream_);
        }
        template <class PingHandler>
        auto
        async_ping(beast::websocket::ping_data const& payload, PingHandler&& handler)
        {
            return std::visit([&, handler = std::forward<PingHandler>(handler)](auto& t) mutable
                              { return t.async_ping(payload, std::forward<PingHandler>(handler)); },
                              stream_);
        }
        template <class PongHandler>
        auto
        async_pong(beast::websocket::ping_data const& payload, PongHandler&& handler)
        {
            return std::visit([&, handler = std::forward<PongHandler>(handler)](auto& t) mutable
        ...
        websocket_stream(stream_t&& stream) : stream_(std::move(stream)) {}
