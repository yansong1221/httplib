...
static void
_localtime(struct tm* _Tm, const time_t* _Time)
{
#ifdef _WIN32
...
static void
_gmtime(struct tm* _Tm, const time_t* _Time)
{
#ifdef _WIN32
    gmtime_s(_Tm, _Time);
#else
...
cast(const fs::file_time_type& endpoint)
{
...
make_unc_path(const fs::path& path)
{
...
file_last_wirte_time(const fs::path& file)
{
...
std::string
format_dir_to_html(std::string_view target,
                   const fs::path& path,
                   boost::system::error_code ec)
{
    ...
    std::string body =
        fmt::format((const char*)detail::body_fmt, "../", "../", "", "", "");
...
std::time_t
file_last_write_time(const fs::path& path, std::error_code& ec)
{
...
std::string
format_http_current_gmt_date()
{
    return format_http_gmt_date(time(nullptr));
}

std::string
format_http_gmt_date(const std::time_t& time)
{
...
std::time_t
parse_http_gmt_date(const std::string& http_date)
{
    std::tm tm = {};
    ...
    if (iss.fail()) { throw std::runtime_error("Failed to parse HTTP date string"); }
...
http_ranges
parser_http_ranges(std::string_view range_str, size_t file_size, bool& is_valid) noexcept
{
    is_valid  = true;
    ...
    if (range_str == "-") { return {{0, file_size - 1}}; }
...
query_params
parse_http_query_params(std::string_view content, bool& is_valid)
{
    is_valid = true;
...
std::string
make_http_query_params(const query_params& params)
{
