    ...
    class chunk_reader_core
    {
      public:
              ...
              std::chrono::steady_clock::duration read_timeout)
        {
            ...
            auto ctx = ctx_;
            ctx_->chunk_cb = [ctx](std::uint64_t /*remain*/, std::string_view body, beast::error_code&) -> std::size_t
            {
        ...
        read_chunk()
        {
            ...
            while (ctx_->chunks.empty() && !ctx_->done)
            {
                ...
                co_await http::async_read_some(*ctx_->stream, *ctx_->buffer, *ctx_->parser, util::net_awaitable[ec]);
        ...
        is_done() const
        {
            return ctx_ && ctx_->done && ctx_->chunks.empty();
        }
