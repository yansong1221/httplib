...
class router {
public:
    explicit router(std::shared_ptr<spdlog::logger> logger) : logger_(logger) {}
...
public:
    ...
    void set_http_handler(http::verb method, std::string_view key,
        ...
        if (map.count(method)) {
            logger_->warn("router method: {} key: {} has already registered.",
                          http::to_string(method), key);
            return;
        }
...
private:
    ...
    std::vector<httplib::router::mount_point_entry> static_file_entry_;
};
