...
public:
    ...
    virtual void
    init(boost::system::error_code& ec) = 0;
...
public:
    ...
    virtual std::size_t
    put(any_body::reader::const_buffers_type const& buffers,
        boost::system::error_code& ec) = 0;
    virtual void
    finish(boost::system::error_code& ec) = 0;
};
...
public:
    ...
    void
    init(boost::system::error_code& ec) override
    {
        writer_.init(ec);
    };
...
public:
    ...
    {
        compressor_ =
            compressor_factory::instance().create(header[http::field::content_encoding]);
    }
    void
    ...
    create_proxy_writer(http::fields& h, any_body::variant_value<Bodies...>& body)
    {
        return std::visit(
            [&](auto& t) -> std::unique_ptr<detail::proxy_writer> {
                ...
                // 提取匹配的 Body 类型
                using body_type =
                    typename any_body::match_body<value_type, Bodies...>::type;
...
public:
    ...
    finish(boost::system::error_code& ec)
    {
        ...
        compressor_->finish();
        auto decoded_buffer = compressor_->buffer();
        if (decoded_buffer.size() != 0) { proxy_->put(decoded_buffer, ec); }
        return proxy_->finish(ec);
    }

private:
    template<class Body>
    std::unique_ptr<detail::proxy_reader>
    create_proxy_reader(http::fields& h, any_body::value_type& body)
    {
        return std::visit(
...
void
any_body::reader::init(boost::optional<std::uint64_t> const& content_length,
                       boost::system::error_code& ec)
{
