...
std::optional<form_field_data> form_data::field_by_name(const std::string_view field_name) const {
    const auto &it = std::find_if(std::cbegin(fields), std::cend(fields),
...
bool form_data::has_data(const std::string_view field_name) const {
    return field_by_name(field_name).has_value();
}

bool form_data::has_content(const std::string_view field_name) const {
    // Retrieve field
...
std::optional<std::string> form_data::content(const std::string_view field_name) const {
    // Retrieve field
    const auto &field = field_by_name(field_name);
    if (!field)
        return {};
