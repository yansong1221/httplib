...
public:
    ...
    http::request<body::any_body>
    make_http_request(http::verb method,
...
public:
    ...
    async_send_request(http::request<body::any_body>& req)
    {
    ...
    async_send_request(http::request<body::any_body>& req, client::response& resp)
    {
        try {
...
void
client::set_timeout_policy(const timeout_policy& policy)
{
    impl_->set_timeout_policy(policy);
}

void
client::set_timeout(const std::chrono::steady_clock::duration& duration)
{
    impl_->set_timeout(duration);
}

void
client::set_use_ssl(bool ssl)
{
...
client::async_head(std::string_view path,
                   const http::fields& headers /*= http::fields()*/)
{
...
client::get(std::string_view path,
            const html::query_params& params,
            const http::fields& headers /*= http::fields()*/)
{
