...
template<class T>
struct has_before<T,
                  std::void_t<decltype(std::declval<T>().before(std::declval<request&>(), std::declval<response&>()))>>
    : std::true_type
{
};
...
template<class T>
struct has_after<T, std::void_t<decltype(std::declval<T>().after(std::declval<request&>(), std::declval<response&>()))>>
    : std::true_type
{
};
...
coro_http_handler_type create_router_coro_http_handler(Func&& handler, Aspects&&... asps)
{
    ...
    {
        http_handler = [handler_variant = std::move(handler_variant), ... asps = std::forward<Aspects>(asps)](
    ...
    {
        http_handler = [handler_variant = std::move(handler_variant)](request& req,
                                                                      response& resp) mutable -> net::awaitable<void>
        { co_await handler_variant(req, resp); };
    }
...
template<typename Func, typename... Aspects>
void router::set_http_handler(http::verb method, std::string_view key, Func&& handler, Aspects&&... asps)
{
    coro_http_handler_type http_handler = detail::create_router_coro_http_handler(std::move(handler), asps...);

    set_http_handler(method, key, std::move(http_handler));
}

template<http::verb... method, typename Func, typename... Aspects>
void router::set_http_handler(std::string_view key, Func&& handler, util::class_type_t<Func>& owner, Aspects&&... asps)
{
    ...
    {
        coro_http_handler_type f = std::bind(handler, &owner, std::placeholders::_1, std::placeholders::_2);
        set_http_handler<method...>(key, std::move(f), std::forward<Aspects>(asps)...);
    }
    else
    {
        http_handler_type f = std::bind(handler, &owner, std::placeholders::_1, std::placeholders::_2);
        set_http_handler<method...>(key, std::move(f), std::forward<Aspects>(asps)...);
    }
}
...
{
    auto coro_handler = detail::create_router_coro_http_handler(std::move(handler), std::forward<Aspects>(asps)...);
    set_default_handler(std::move(coro_handler));
}
...
{
    auto coro_handler = detail::create_router_coro_http_handler(std::move(handler), std::forward<Aspects>(asps)...);
    set_file_request_handler(std::move(coro_handler));
}
