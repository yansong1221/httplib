...
public:
    client(net::io_context& ex, std::string_view host, uint16_t port) : client(ex.get_executor(), host, port) { }
...
public:
    ...
    template<typename ResponseBody = http::string_body>
    auto async_post(std::string_view path, std::string_view body, const http::fields headers = http::fields())
    {
        auto request = make_http_request<http::span_body<const char>>(http::verb::post, path, headers);
        request.body() = body;
    ...
    {
        auto future = net::co_spawn(executor_, async_get<ResponseBody>(path, headers), net::use_future);
        return future.get();
    }
    ...
    net::awaitable<http::response<ResponseBody>> async_send_request(http::request<RequestBody> request)
    {
        try
        {
