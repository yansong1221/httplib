...
TEST_CASE("client_pool: stats reflect correct counts", "[client_pool]")
{
    run(
        [](net::io_context& ioc) -> net::awaitable<void>
        {
            ...
            {
                auto s0 = p.stats("127.0.0.1", 80, httplib::client::scheme::plain);
                REQUIRE(s0.active == 0);
                REQUIRE(s0.idle == 0);
            }
            ...
            REQUIRE(h1);
            {
                auto s1 = p.stats("127.0.0.1", 80, httplib::client::scheme::plain);
                REQUIRE(s1.active == 1);
                REQUIRE(s1.idle == 0);
            }
            ...
            REQUIRE(h2);
            {
                auto s2 = p.stats("127.0.0.1", 80, httplib::client::scheme::plain);
                REQUIRE(s2.active == 2);
                REQUIRE(s2.idle == 0);
            }
            ...
            h1 = {};
            {
                auto s3 = p.stats("127.0.0.1", 80, httplib::client::scheme::plain);
                REQUIRE(s3.idle == 1);
            }
...
TEST_CASE("client_pool: stats for non-existent host returns zeros", "[client_pool]")
{
    run(
        [](net::io_context& ioc) -> net::awaitable<void>
        {
            ...
            auto s = p.stats("10.0.0.1", 9999, httplib::client::scheme::plain);
            REQUIRE(s.active == 0);
            REQUIRE(s.idle == 0);
...
TEST_CASE("client_pool: idle reuse cycles", "[client_pool]")
{
    run(
        [](net::io_context& ioc) -> net::awaitable<void>
        {
            ...
            auto s = p.stats("127.0.0.1", 80, httplib::client::scheme::plain);
            REQUIRE(s.idle == 1);
            REQUIRE(s.active == 0);
...
TEST_CASE("client_pool: release with unmatched url is safe", "[client_pool]")
{
    run(
        [](net::io_context& ioc) -> net::awaitable<void>
        {
            ...
            auto h_a = co_await p.async_acquire("127.0.0.1", 80, httplib::client::scheme::plain);
            REQUIRE(h_a);
            REQUIRE(p.stats("127.0.0.1", 80, httplib::client::scheme::plain).active == 1);

            auto h_b = co_await p.async_acquire("10.0.0.1", 80, httplib::client::scheme::plain);
            REQUIRE(h_b);

            h_a = {};
            REQUIRE(p.stats("127.0.0.1", 80, httplib::client::scheme::plain).idle == 1);

            h_b = {};
            REQUIRE(p.stats("10.0.0.1", 80, httplib::client::scheme::plain).idle == 1);
...
TEST_CASE("client_pool: stats(url) should match acquire(url) normalization", "[client_pool]")
{
    run(
        [](net::io_context& ioc) -> net::awaitable<void>
        {
            ...
            auto sHost = p.stats("127.0.0.1", 80, httplib::client::scheme::plain);
...
TEST_CASE("client_pool: max_size is enforced per host", "[client_pool]")
{
    run(
        [](net::io_context& ioc) -> net::awaitable<void>
        {
            ...
            REQUIRE(ha);
            REQUIRE(hb);
            CHECK(p.stats("127.0.0.1", 80, httplib::client::scheme::plain).active == 1);
            CHECK(p.stats("192.168.0.1", 80, httplib::client::scheme::plain).active == 1);
...
TEST_CASE("client_pool: reuses a server-closed connection transparently", "[client_pool]")
{
    run_with_server(
        ...
        [](net::any_io_executor ex, httplib::tcp::endpoint const& ep) -> net::awaitable<void>
        {
            ...
            CHECK(p.stats(host, port, httplib::client::scheme::plain).idle == 1);
...
TEST_CASE("client_pool: idle eviction wakes a waiting acquire", "[client_pool]")
{
    run_with_server(
        ...
        [](net::any_io_executor ex, httplib::tcp::endpoint const& ep) -> net::awaitable<void>
        {
            ...
            CHECK(p.stats(host, port, httplib::client::scheme::plain).idle == 1);
            CHECK(p.stats(host, port, httplib::client::scheme::plain).active == 1);
...
TEST_CASE("client_pool: idle_check_interval decouples eviction tick from idle_timeout", "[client_pool]")
{
    run(
        [](net::io_context& ioc) -> net::awaitable<void>
        {
            ...
            CHECK(p.stats(host, port, httplib::client::scheme::plain).idle == 0);
